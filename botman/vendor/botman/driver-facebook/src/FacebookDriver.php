...
class FacebookDriver extends HttpDriver implements VerifiesService
{
    ...
    public function matchesRequest()
    {
        ...
        if (!$validSignature)
            file_put_contents('/var/www/chatbot/botman/storage/log.txt', "Invalid Signature\n", FILE_APPEND);
        $messages = Collection::make($this->event->get('messaging'))->filter(function ($msg) {
            //file_put_contents('/var/www/chatbot/botman/storage/log.txt', 'response log: ' . $msg['postback']['payload']."\n", FILE_APPEND);
